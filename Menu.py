...
class Menu:
    def __init__(self, screen, maps, color_word):
        self.screen = screen
        self.maps = maps
    ...
    def render(self, width, height):
        ...
        for i in self.maps:
            ...
            text = font1.render(i[1], 1, self.color_word)
            self.screen.blit(text, [width - 580, height // 2 - 40])

            font2 = pygame.font.Font(None, 40)
            text = font2.render(i[2], 1, self.color_word)
            self.screen.blit(text, [width - 540, height // 2 + 40])

            font3 = pygame.font.Font(None, 60)
            text1 = font3.render('Топ набранных очков', 1, self.color_word)
            text2 = font3.render('на карте', 1, self.color_word)
            ...
            for j in list_scores:
                text = font1.render(str(j), 1, self.color_word)
